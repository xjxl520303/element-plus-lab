...
Element Plus Lab 是一个专注于分享基于 **Vue 3** 和 **Element Plus** 的业务组件封装示例和演示的项目，并发布为 NPM 包 [element-plus-lab](https://www.npmjs.com/package/element-plus-lab)。
...
本项目不仅是一个组件库，也是一个学习、交流和参考的资源库，帮助开发者快速上手 Vue 3 + Element Plus 的开发模式，提升开发效率。项目不一定全部都是 Element Plus 的封装，也会涉及一些第三方库的集成使用，但宗旨是紧扣实际业务、以业务为导向。

项目也会参考并收集社区中一些优秀的业务领域组件分享文章和开源的 Admin 项目中提供的实用组件进行整合和演示。

## 包内能力概览

安装 `element-plus-lab` 后可使用：

- **钩子**：`useDialog`、`useDrawer`（Promise 化对话框/抽屉，需配合 portal-vue 传送目标）
- **Vue 组件**：`ElFormGroup`、`ElSimplePageHeader`、`ElSimpleSteps`（支持 `app.use(ElementPlusLab)` 全量注册或 `app.use(ElFormGroup)` 等按需注册）

详细安装与使用方式见 [快速开始](/guide/introduction/quick-start)。
...
项目后面有尝试过基于 [vue3-turbo-component-lib-template](https://github.com/huangmingfu/vue3-turbo-component-lib-template) 进行重新实现（基于现有的工程化方案，只专注于业务代码的实现）。这里感谢作者的付出，最终尝试后还是放弃这个方案了，而采用参考 Vben Admin 和 Element Plus 从零重新实现，并在此基础上不断完善功能。
